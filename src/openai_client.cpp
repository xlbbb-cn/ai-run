...
std::string OpenAIClient::generate_command(const std::string& system_prompt,
                                            const std::string& user_query) {
    if (api_key_.empty())
        throw std::runtime_error("OpenAI API key is not configured");
    ...
    try {
        ...
        throw std::runtime_error("OpenAI: invalid JSON response: " + raw);
    }
    ...
    std::string cmd = resp["choices"][0]["message"]["content"].get<std::string>();
